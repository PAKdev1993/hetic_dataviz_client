...
import jsonData from "../charts/LiquidFill.json";
import * as d3 from "d3";

const newData = Object.keys(jsonData["Dataviz3.2"][0]).map(key => {
  return {
    key,
    value: parseFloat(jsonData["Dataviz3.2"][0][key])
  };
});
console.log(d3);
export default class LiquidFill extends React.Component {
  ...
  componentDidMount() {
    const id = this.chartRef.current.id;
    console.log(newData[0].value);
    this.loadLiquidFillGauge(id, newData[0].value);
  }
  ...
  render() {
    return <svg ref={this.chartRef} id="fillgauge1" width="97%" height="250" />;
  }
}
